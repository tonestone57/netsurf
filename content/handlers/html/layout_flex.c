		...
		struct flex_item_data *item)
{
	...
	NSLOG(flex, DEEPDEBUG, "flex-item box: %p: "
			"Frozen at target_main_size: %i", (void *)item->box, item->target_main_size);
}
		...
		struct flex_line_data *line)
{
	...
	NSLOG(flex, DEEPDEBUG, "box %p: line %" PRIsizet ": first: %" PRIsizet ", count: %" PRIsizet "", (void *)ctx->flex, (size_t)(line - ctx->line.data),
		...
		struct flex_ctx *ctx)
{
	...
	while (pos < ctx->item.count) {
		...
		NSLOG(flex, DEEPDEBUG, "flex-container: %p: "
				"fitted: %" PRIsizet " (total: %" PRIsizet "/%" PRIsizet ")", (void *)ctx->flex, line->count,
				pos, ctx->item.count);
		...
		html_content *content)
{
	...
	NSLOG(flex, DEEPDEBUG, "box %p: %s, available_width %i, width: %i", (void *)flex, ctx->horizontal ? "horizontal" : "vertical",
