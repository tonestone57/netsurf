...
void js_event_cleanup(struct jsthread *thread, struct dom_event *evt)
{
	...
	/* Remove the event from the memoization map to allow GC of the JS wrapper.
	 * QuickJS finalizers (e.g. qjsky_event_finalizer) will then handle
	 * the deallocation of C-level event resources.
	 */
	JSContext *ctx = thread->ctx;
	struct jsheap *heap = thread->heap;
	JSValue global = JS_GetGlobalObject(ctx);
	JSValue map = JS_GetProperty(ctx, global, heap->node_map_atom);

	if (JS_IsObject(map)) {
		JSValue key = JS_NewBigUint64(ctx, (uint64_t)(uintptr_t)evt);
		JSValue delete_fn = JS_GetPropertyStr(ctx, map, "delete");
		if (JS_IsFunction(ctx, delete_fn)) {
			JSValue ret = JS_Call(ctx, delete_fn, map, 1, &key);
...
js_dom_event_add_listener(jsthread *thread,
			  ...
			  void *js_funcval)
{
	JSValue func = JS_DupValue(thread->ctx, *(JSValue *)js_funcval);
	qjsky_add_event_listener(thread->ctx, node, event_type_dom, func, false);
	return true;
}
