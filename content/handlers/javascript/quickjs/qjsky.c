...
static void qjsky_node_finalizer(JSRuntime *rt, JSValue val)
{
	...
	if (node) {
		/* Clear the weak reference from the native node if key is still valid */
		if (heap->node_js_wrapper_key) {
			void *dummy;
			dom_node_set_user_data(node, heap->node_js_wrapper_key, NULL, NULL, &dummy);
		}
		dom_node_unref(node);
	}
}
...
{
	if (heap->node_js_wrapper_key) {
		dom_string_unref(heap->node_js_wrapper_key);
