...
static void qjsky_node_finalizer(JSRuntime *rt, JSValue val)
{
	...
	if (!node) node = JS_GetOpaque(val, heap->document_class_id);
	if (!node) node = JS_GetOpaque(val, heap->element_class_id);
...
static void qjsky_event_finalizer(JSRuntime *rt, JSValue val)
{
	...
	if (!evt) evt = JS_GetOpaque(val, heap->uievent_class_id);
	if (!evt) evt = JS_GetOpaque(val, heap->mouseevent_class_id);
	if (!evt) evt = JS_GetOpaque(val, heap->keyboardevent_class_id);
...
static JSValue qjsky_keyboardevent_get_key(JSContext *ctx, JSValueConst this_val)
{
	...
	JSValue val = JS_NULL;
	if (key) {
		val = qjsky_dom_string_to_js_value(ctx, key);
...
static void qjsky_generic_event_handler(dom_event *evt, void *pw)
{
	...
	/* 1. Handle inline handlers (from the "handlerMap" property) */
	dom_event_flow_phase phase;
	dom_event_get_event_phase(evt, &phase);

	JSAtom prop = JS_NewAtomLen(ctx, (const char *)dom_string_data(name), dom_string_byte_length(name));

	if (phase != DOM_EVENT_CAPTURING_PHASE) {
		JSValue handler_map = JS_GetProperty(ctx, node_val, heap->handler_map_atom);
		JSValue handler = JS_GetProperty(ctx, handler_map, prop);

		if (JS_IsFunction(ctx, handler)) {
			JSValue ret = JS_Call(ctx, handler, node_val, 1, &event_val);
			if (JS_IsException(ret)) qjs_log_exception(ctx, "Inline event handler error");
	...
	/* 2. Handle programmatic listeners (from the "handlerListenerMap" property) */
	JSValue listener_map = JS_GetProperty(ctx, node_val, heap->handler_listener_map_atom);
	JSValue listeners = JS_GetProperty(ctx, listener_map, prop);

	if (JS_IsArray(ctx, listeners)) {
		uint32_t len = 0;
		JSValue js_len = JS_GetPropertyStr(ctx, listeners, "length");
		JS_ToUint32(ctx, &len, js_len);
		JS_FreeValue(ctx, js_len);

		for (uint32_t i = 0; i < len; i++) {
			JSValue entry = JS_GetPropertyUint32(ctx, listeners, i);
			JSValue js_capture = JS_GetPropertyStr(ctx, entry, "capture");
			bool capture = JS_ToBool(ctx, js_capture);
			JS_FreeValue(ctx, js_capture);

			bool run = false;
			if (capture && phase == DOM_EVENT_CAPTURING_PHASE) run = true;
			if (!capture && (phase == DOM_EVENT_AT_TARGET || phase == DOM_EVENT_BUBBLING_PHASE)) run = true;

			if (run) {
				JSValue listener = JS_GetPropertyStr(ctx, entry, "func");
				if (JS_IsFunction(ctx, listener)) {
					JSValue ret = JS_Call(ctx, listener, node_val, 1, &event_val);
					if (JS_IsException(ret)) qjs_log_exception(ctx, "Programmatic event listener error");
					JS_FreeValue(ctx, ret);
				}
...
void qjsky_add_event_listener(JSContext *ctx, struct dom_node *node, dom_string *type, JSValue func, bool capture)
{
	struct jsheap *heap = JS_GetRuntimeOpaque(JS_GetRuntime(ctx));
	JSValue node_obj = qjsky_push_node(ctx, node);
	JSValue listener_map = JS_GetProperty(ctx, node_obj, heap->handler_listener_map_atom);

	if (JS_IsUndefined(listener_map)) {
		listener_map = JS_NewObject(ctx);
		JS_SetProperty(ctx, node_obj, heap->handler_listener_map_atom, JS_DupValue(ctx, listener_map));
	}

	JSAtom prop = JS_NewAtomLen(ctx, (const char *)dom_string_data(type), dom_string_byte_length(type));

	JSValue listeners = JS_GetProperty(ctx, listener_map, prop);
	if (!JS_IsArray(ctx, listeners)) {
		JS_FreeValue(ctx, listeners);
		listeners = JS_NewArray(ctx);
		JS_SetProperty(ctx, listener_map, prop, JS_DupValue(ctx, listeners));
	}
	...
	JS_SetPropertyUint32(ctx, listeners, len, func);

	JS_FreeAtom(ctx, prop);
	JS_FreeValue(ctx, listeners);
	JS_FreeValue(ctx, listener_map);
	JS_FreeValue(ctx, node_obj);
	...
	dom_event_target_add_event_listener((dom_event_target *)node, type, listen, capture);
	dom_event_listener_unref(listen);
}
...
void qjsky_init_context(JSContext *ctx)
{
	...
	if (heap->node_map_atom == JS_ATOM_NULL) {
		...
		heap->node_proto_atom = JS_NewAtom(ctx, "__nodeProto");
		...
		JS_SetPrototype(ctx, document_proto, JS_DupValue(ctx, node_proto));
		heap->document_proto_atom = JS_NewAtom(ctx, "__documentProto");
		JS_SetProperty(ctx, global, heap->document_proto_atom, JS_DupValue(ctx, document_proto));
		JS_SetClassProto(ctx, heap->document_class_id, JS_DupValue(ctx, document_proto));

		JSValue element_proto = JS_NewObject(ctx);
		JS_SetPrototype(ctx, element_proto, JS_DupValue(ctx, node_proto));
		heap->element_proto_atom = JS_NewAtom(ctx, "__elementProto");
		JS_SetProperty(ctx, global, heap->element_proto_atom, JS_DupValue(ctx, element_proto));
		JS_SetClassProto(ctx, heap->element_class_id, JS_DupValue(ctx, element_proto));
